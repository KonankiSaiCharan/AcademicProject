###This code has been used to evaluate different modeling techniques and select the best technique 
###which gives the required efficiency in prediction of internet connectivity status at a students 
###home
...
LogisticStatistics <- sapply(thresholdValues, function(x) {
  ...
  negativePrecitedValue <- sum(cl == Train.Student.Data$internet & cl=="no")/sum(cl == "no")
  trueNegativeRate <- sum(cl == Train.Student.Data$internet & cl=="no")/sum(Train.Student.Data$internet == "no")
  return(c(x,accuracy,negativePrecitedValue,trueNegativeRate))
})

LogisticStatistics <- as.data.frame(t(LogisticStatistics))
colnames(LogisticStatistics) <- c("threshold","accuracy","negativePrecitedValue","trueNegativeRate")

ggplot(LogisticStatistics,aes(y=seq(from=0,to=1, by=0.1))) + 
  geom_point(aes(x=threshold,y=accuracy,color="accuracy")) +
  geom_point(aes(x=threshold,y=negativePrecitedValue,color="negative precited value")) +
  geom_point(aes(x=threshold,y=trueNegativeRate,color="true negative rate"))+
  geom_vline(xintercept = 0.675)+ 
  labs(x="Threshold", y="Proportion", title=element_text("Threshold vs Accuracy, NPV and TNR")) +
  theme(legend.text=element_text(size=9), legend.position = "bottom")

#negative predicted value and true negative rate
...
ggplot(LogisticStatistics,aes(y='%values')) + 
  geom_point(aes(x=threshold,y=accuracy,color="accuracy")) +
  geom_point(aes(x=threshold,y=negativePrecitedValue,color="negative precited value")) +
  geom_point(aes(x=threshold,y=trueNegativeRate,color="true negative Rate")) +
  geom_vline(xintercept = 0.742)+ theme(legend.position="bottom") + 
  labs(x="Threshold", y="Proportion", title=element_text("Threshold vs Accuracy, NPV and TNR")) +
  theme(legend.text=element_text(size=9), legend.position = "bottom")

#LASSO subset selection best threshold is 0.74
STHLASSO  <- LogisticStatistics[LogisticStatistics$threshold == 0.742,]
#@@@
...
plot(lda.perf, colorize=T, main="LDA",xlab ="Negative Precited Value",ylab="True Negative Rate")
...
ggplot(LdaStatistics,aes(y=seq(from=0,to=1, by=0.1))) + 
  geom_point(aes(x=threshold,y=accuracy,color="accuracy")) +
  geom_point(aes(x=threshold,y=precision,color="Negative Precited Value")) +
  geom_point(aes(x=threshold,y=truePositiveRate,color="True Negative Rate")) +
